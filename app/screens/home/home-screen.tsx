...
import { ViewStyle, SafeAreaView, StyleSheet, TouchableOpacity, View, Image } from "react-native"
...
export const HomeScreen = observer(function HomeScreen({ navigation }) {
  ...
  const initialCurrentLocation = {
    streetName: "Tewa Street",
    gps: {
      latitude: -45.011484,
      longitude: 168.745961,
    },
  }
  ...
  const restaurantData = [
    {
      ...
      duration: "30 - 45 min",
      location: {
        latitude: -45.03115051985585,
        longitude: 168.65844668926212,
      },
      courier: {
          ...
          <View style={{ marginTop: SIZES.padding, flexDirection: "row" }}>
            ...
            <View style={{ flexDirection: "row", marginLeft: 10 }}>
              {item.categories.map((categoriId) => {
                return (
                  <View style={{ flexDirection: "row" }} key={categoriId}>
                    <Text style={{ ...FONTS.body3 }}>{getCategoryNameById(categoriId)}</Text>
